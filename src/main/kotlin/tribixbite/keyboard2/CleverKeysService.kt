...
class CleverKeysService : InputMethodService(),
    ...
    // Note: VibratorCompat is a Kotlin object (singleton) - no property needed
    ...
    override fun onCreate() {
        ...
        try {
            ...
            logD("✅ CleverKeys initialization complete")
        } catch (e: Exception) {
            logE("❌ Initialization failed", e)
            e.printStackTrace()
            throw RuntimeException("CleverKeys initialization failed: ${e.message}", e)
        }
    }
    ...
    // Note: VibratorCompat is a Kotlin object (singleton) - uses static vibrate() method directly
    ...
    // ============================================================================
    // Build Configuration Constants (12 lines)
    // ============================================================================

    /**
     * Initialize BuildConfig object (singleton)
     *
     * Build configuration constants generated at build time.
     * This is a stub for compilation - actual values generated by Gradle.
     *
     * Constants:
     * - DEBUG: Boolean flag for debug build (true)
     * - APPLICATION_ID: Application identifier ("tribixbite.keyboard2")
     * - BUILD_TYPE: Build type string ("debug")
     * - VERSION_CODE: Numeric version code (50)
     * - VERSION_NAME: Semantic version string ("1.32.1")
     *
     * Features:
     ...
     *
     * Usage:
     * - BuildConfig.DEBUG: Check if debug build
     * - BuildConfig.VERSION_NAME: Display version to user
     * - BuildConfig.VERSION_CODE: Compare versions
     * - BuildConfig.APPLICATION_ID: Package name reference
     * - BuildConfig.BUILD_TYPE: Runtime build type detection
     *
     * Kotlin object (singleton) - no instantiation needed.
     * Constants available via BuildConfig.* references.
     */
    private fun initializeBuildConfig() {
        try {
            // BuildConfig is a Kotlin object (singleton) - no instantiation needed
            // Constants are available via BuildConfig.* references

            logD("✅ BuildConfig object initialized (12 lines)")
            logD("   - Build Configuration:")
            logD("     * DEBUG: ${BuildConfig.DEBUG}")
            logD("     * APPLICATION_ID: ${BuildConfig.APPLICATION_ID}")
            logD("     * BUILD_TYPE: ${BuildConfig.BUILD_TYPE}")
            logD("     * VERSION_CODE: ${BuildConfig.VERSION_CODE}")
            logD("     * VERSION_NAME: ${BuildConfig.VERSION_NAME}")
            logD("   - Constants:")
            logD("     * DEBUG: Boolean flag for debug build")
            logD("     * APPLICATION_ID: Application identifier")
            logD("     * BUILD_TYPE: Build type string")
            logD("     * VERSION_CODE: Numeric version")
            logD("     * VERSION_NAME: Semantic version")
            logD("   - Features:")
            logD("     * Compile-time constants (const val)")
            logD("     * Gradle build integration")
            logD("     * Version tracking")
            logD("     * Build type detection")
