...
| `finger_occlusion_offset` | Signed % of one key row added to raw touch Y before the letter-box affine — fingertip-occlusion compensation. | **0** (`Config.kt:148,606`), validated −25..25 (`SettingsValidation.kt:250`), applied `CtcEngineAdapter.kt:726-741`. | **Yes — already exposed**, default-off. | **Already an offset knob — this is the template.** Signed, bounded, unit tied to geometry (% of a key row), default = do nothing. | Deliberately did NOT inherit the removed neural engine's 12.5 %. |
...
| a–z projection policy | CKDT lexicons: NFD → strip combining marks → a–z; no-a–z-spelling words (ß, œ) DROPPED; canonical form kept for display (`CtcEngineAdapter.kt:493-505`). | Fixed policy — the λ-sweep lexicons were built with exactly this projection (`:497`). | **No.** Changing it invalidates the per-language λ evidence and the display map simultaneously. | n/a | A different projection is a new evaluation campaign, not a setting. |
