...
| Item | Area | Notes |
|---|---|---|
| ~~**UT-1** System light theme doesn't apply to Dictionary Manager~~ ✅ fixed 2026-07-15, pending user visual approval | Theming (#35 residue) | DictionaryManagerActivity was intentionally left with DayNight window bg + self-consistent dark panels in `787913453`; user rejected that — fixed via day/night color resources (`res/values/colors.xml` + `res/values-night/colors.xml`, night values byte-identical to the old hardcoded dark hexes) + toolbar overlay fix. See Addendum. |
| ~~**UT-2** Possessives incorrectly trigger "add to dictionary" prompt~~ ✅ fixed 2026-07-15 | Suggestions/dictionary | Prompt decision extracted to pure `AutocorrectContextGuard.shouldOfferAddToDictionary`; a token whose apostrophe base (`'s`/bare-`'`, U+0027 or U+2019, mirroring autocorrect step 1.6) is a known non-disabled word no longer prompts. See Addendum. |
| ~~**UT-3** Some URL edits trigger "add to dictionary" prompt~~ ✅ fixed 2026-07-15 | Suggestions/dictionary | Same extracted decision function gates the prompt on the existing `AutocorrectContextGuard.isNonProseContext` result (already computed in SuggestionHandler for the autocorrect path) — URL/email/path tokens never prompt. See Addendum. |
| ~~**UT-4** Whitespace/newline-only clipboard edits don't save~~ ✅ fixed 2026-07-15 | Clipboard | Hypothesis confirmed exactly: BOTH `ClipboardHistoryService.editEntryContent` (:438) and `ClipboardDatabase.updateEntryContentInTable` (:733) compared `.trim()`-ed old vs new and returned `Success` without writing — and the DB layer stored `trimmedNew` anyway, so leading/trailing whitespace could never persist (all three tabs route through these two functions; the #108 insert-dedup path is not involved). Fixed via shared pure `ClipboardEditPolicy.decide` (exact comparison, verbatim storage) + exact-first `resolveContentKey` fallback so edited entries stay deletable/unpinnable/taggable. See Addendum. |
| **UT-5** "doesn't" nearly impossible to swipe — **deferred to v1.5.1 by user decision 2026-07-15** | Neural/vocab | Apostrophe words depend on contraction mapping from swiped `doesnt`; ranking appears too weak to surface `doesn't` |
| ~~**UT-6** Debug-app FRE splash "Select keyboard" soft-resets the phone~~ ✅ mitigated 2026-07-15 | FRE/Launcher — was RELEASE BLOCKER | Root cause: `showInputMethodPicker()` is a binder call into system_server; on some OEM ROMs it crashes InputMethodManagerService (→ soft reset) when issued from an unfocused window / before the IME is enabled — app-side try/catch cannot intercept a system_server crash. Mitigation in LauncherActivity.launchInputMethodPicker(): IME-not-enabled → route to IME settings Activity instead of picker; otherwise defer via decorView.post and re-check hasWindowFocus(), falling back to IME settings if unfocused. New LauncherSelectKeyboardSafetyTest (2 instrumented cases) drives the real button path. USER-CONFIRMED 2026-07-15: soft reset still occurs on the affected device but was diagnosed as a DEVICE-level fault, not CleverKeys — the defensive mitigation is retained (removes every risky call state the app controls); not a release blocker |
| **UT-7** "I'd" needs manual score boost, especially after periods — **deferred to v1.5.1 by user decision 2026-07-15** (tackle with UT-5 as one contraction-ranking work item) | Neural/vocab | `I'd` under-ranked in predictions, notably at sentence start after `.` where capitalized `I'd` should be likelier |
...
| **#35** Settings app follows system day/night (over-dark dark mode) | (this commit) | 2026-07-13 | Architecture finding: the MAIN settings UI was already fixed in 9213de835 (2026-01) — SettingsActivity + 5 sibling activities wrap content in `KeyboardTheme`, which defaults `darkTheme = isSystemInDarkTheme()`, and `MaterialThemeManager` already ships a branded light scheme (+ Material You dynamic light on API 31+); the whole `ui/settings/` tree is 100% `MaterialTheme.colorScheme` tokens (zero hardcoded colors). Residual offenders fixed now: **ThemeSettingsActivity** (hardcoded purple/blue `darkColorScheme` → system-selected; dark branch byte-identical, new derived light scheme with same accent hues darkened for contrast — primary 0xFF7D3C98, secondary 0xFF1976D2, neutrals matching MaterialThemeManager's light scheme), **ShortSwipeCustomizationActivity** + **ExtraKeysConfigActivity** (`darkColorScheme()` → `if (isSystemInDarkTheme()) darkColorScheme() else lightColorScheme()`). Intentionally NOT changed: LauncherActivity (forced-dark matrix look, by design), the keyboard's own theme (user-selected pref, out of scope), SwipeCalibration/SwipeDebug/DictionaryManager (View-based debug/utility screens; DayNight window bg + self-consistent dark panels, readable in both modes). Compile-gated only — **⚠️ #35 REQUIRES USER VISUAL APPROVAL (light-mode pass through all settings screens + dark-mode no-op check) before it can be called fixed**; per testing policy no adb/screenshot verification was performed. |
| **#151** Suggestion tap leaves partial word behind in URL bars (`exa` + tap `example` → `exa example `) | (this commit) | 2026-07-13 | REPRODUCED via code-path trace: `PredictionContextTracker.shouldSyncForInputType()` skips cursor-sync for `TYPE_TEXT_VARIATION_URI`/email/password fields, so `getCharsToDeleteForPrediction()` is always (0,0) there, and the #78 editor-scan fallback was gated on `getCurrentWordLength()==0` — but currentWord IS tracked from typing (`exa`), so the fallback never fired and the typed partial was never deleted; `needsSpaceBefore` then saw `a` and injected a leading space → exactly the reported `exa example `. Fix (SuggestionHandler.onSuggestionSelected): (a) the fallback also fires when the field's input type suppresses sync (`syncSuppressedField`), (b) no leading space is ever injected in such fields (after replacing `exa` in `https://exa` the prev char is `/`; ` example` would corrupt the URL). New Issue151UrlBarSuggestionTapTest (5 cases: bare token, token after `https://`, token after `.`, + 2 plain-field controls) drives the real tap path — `SuggestionHandler.onSuggestionSelected(isManualSelection=true)` exactly as SuggestionBridge invokes it — over a real BaseInputConnection. Compile-gated (ew run pending); 1307 pure + 223 mock green. NOTE: the Termux `helhello` variant in the issue thread is a DIFFERENT mechanism (Termux's IC returns empty `getTextBeforeCursor`, sync then clears currentWord, and there is nothing in the editor to scan) — not addressed here. |
| **UT-8** Disabled word "ans" still not autocorrected (user report 2026-07-14) | (this commit) | 2026-07-15 | Root cause: `autoCorrect` step-1 short-circuit (`dictionary.containsKey(typed)` → return unchanged) never consulted `isWordDisabled`, so a dictionary word the user disabled ("ans", byte 172 in en_enhanced) stayed frozen. TDD: 6 new AutoCorrectEndToEndTest cases verified RED pre-fix (`expected:<an[d]> but was:<an[s]>`) then GREEN. Fix gates the step-1 short-circuit, the morphology-inflection guard, and the possessive-base guard on the typed word/base not being disabled (custom-word override kept via `isWordDisabled`; disabled words remain excluded as correction TARGETS). +2 instrumented AutocorrectTest cases (`ans` disable→`and` via real pref write + reloadDisabledWords, commit()-restored; enabled-`ans` regression) — compile-gated, ew run pending. Local gates: runPureTests 1307, runMockTests 229 (223+6). |
| **UT-1** Dictionary Manager follows system light theme (user report 2026-07-15) | (this commit) | 2026-07-15 | Architecture: pure View/XML screen (`activity_dictionary_manager.xml`, `fragment_word_list.xml`, `item_word_editable.xml`) under `settingsTheme` (Theme.AppCompat.DayNight) — the window bg / word rows / toggle rows / dialogs (framework AlertDialog on the DayNight activity context) already followed the system; the breakage was hardcoded dark hexes on the header/search/tabs/buttons plus a forced `ThemeOverlay.AppCompat.Dark.ActionBar` toolbar (white title on AppCompat-light's near-white `colorPrimary`). Fix: all hardcoded hexes moved to `dict_mgr_*` color resources — `res/values-night/colors.xml` carries the EXACT previous values (#2A2A2A header/tabs, #1E1E1E search+spinner, #FFFFFF text, #888888 hint/secondary, #3A3A3A/#FFFFFF Reset+Edit, #5A3A3A/#FFFFFF Delete) so system-dark is byte-identical; `res/values/colors.xml` carries the light palette per the #35/MaterialThemeManager light scheme (#F5F5F5 header/tabs, #FFFFFF fields, #1A1C1E primary text, #757575 hint, #44474E secondary/unselected-tab/empty text, #E0E0E0/#1A1C1E buttons, #FFCDD2/#410002 delete = light errorContainer pair). Toolbar overlay → neutral `ThemeOverlay.AppCompat.ActionBar` (resolves to the identical white-on-#212121 in night; dark-text-on-light in day). Spinner dropdown, add/edit/confirm dialogs, and SwitchCompat tints are theme-driven and needed no change. Zero Kotlin changes (resource qualifiers do the switching → no new JVM test surface); gates: processDebugResources + compileDebugKotlin green. **⚠️ pending user visual approval in system light + dark no-op check**; per testing policy no adb/screenshot verification. |
| **UT-2 + UT-3** "Add to dictionary?" prompt for valid possessives and URL fragments (user report 2026-07-15) | (this commit) | 2026-07-15 | Prompt fires in `SuggestionHandler.handleRegularTyping` (space-completion branch, was a bare `!isInDictionary && !isUserWord` gate). Decision extracted to pure `AutocorrectContextGuard.shouldOfferAddToDictionary(token, inNonProseToken, isKnownWord, isDisabledWord)`: (1) UT-3 — reuses the `isNonProseContext` flag SuggestionHandler already computes for the autocorrect guard, so URL/email/path tokens never prompt; (2) UT-2 — possessive base check mirroring autocorrect step 1.6 byte-for-byte (last U+0027/U+2019 apostrophe at idx ≥ 2, suffix `s` or empty → base known and not disabled → no prompt). Pinned behaviors: possessive of UNKNOWN base (`embeer's`) still prompts (genuinely unknown; AC-4 usually corrects it first); possessive of DISABLED base still prompts (UT-8 parity — disabling removes possessive protection); non-`'s` suffixes (`ember'll`) unprotected; plain unknown words (`flibbertigib`) still prompt. `WordPredictor.isWordDisabled` widened private→public for the base check. TDD: 10 new AutocorrectContextGuardTest cases verified RED on the behavior-preserving extraction (5 failures: `ember's`/`ember’s`/`rivers'` possessives + `example.com/flibber` URL + `flibber@example.com` email all prompted) then 20/20 GREEN. Gates: compileDebugKotlin, runPureTests 1317 (1307+10), runMockTests 229. |
| **Final ew-cli gate** (batched, post all fixes above) | — | 2026-07-13 | Full instrumented suite on emulator.wtf Pixel7 API 34 (orchestrator, 25m): **1332/1332 passed, 0 failures, 0 skips** (run fe144eed-262d-4c88-95b3-3614808e5c03, ~25 min). All new classes executed: AutocorrectUrlGuardTest 7/7, Issue151UrlBarSuggestionTapTest 5/5, AutocorrectTest 56/56 (incl. the 7 new possessive/elongation/alias/floor-slider cases). This closes the "ew run pending" caveats in the rows above. Local suites at same tree: runPureTests 1307, runMockTests 223. |
| **UT-4** Whitespace/newline-only clipboard edits silently dropped (user report 2026-07-15) | (this commit) | 2026-07-15 | Mechanism (audit hypothesis confirmed): TWO trim layers in the edit pipeline — `ClipboardHistoryService.editEntryContent` (:438) returned `Success` with no DB write when `oldContent.trim() == newContent.trim()`, and `ClipboardDatabase.updateEntryContentInTable` (:733) had the identical guard PLUS stored `trimmedNew` (:754), so leading/trailing whitespace could never persist even past the guards. Only leading/trailing whitespace/newline edits were eaten — internal-spacing changes survive `.trim()` equality and already worked; all three tabs (history/pinned/todo) route through the same two functions; the #108 insert-dedup path is not involved in edits. Fix: shared pure `ClipboardEditPolicy.decide(old,new)` (blank→Invalid, exact-equal→NoOp, else Apply verbatim) drives both layers; content + content_hash stored exactly; dedup + WHERE lookups exact; new `resolveContentKey` (exact-first, trimmed-fallback for legacy trimmed rows) keeps whitespace-edited entries deletable/unpinnable/status-changeable/taggable (`removeClipboardEntry`/`unpinEntry`/`removeTodoEntry`/`setTodoEntryStatus`/`setPinnedEntryTags`/`setTodoEntryTags`). Known scope edge: pin/todo COPY inserts (`pinEntry`/`addTodoEntry`) still trim on insert (capture normalization) — editing whitespace INTO an entry then pinning it stores a trimmed copy. TDD: policy extracted verbatim-buggy first, 9 new ClipboardEditJvmTest cases run → RED (3 failures: trailing-newline, leading-whitespace, whitespace-removal all classified `NoOp`), then exact semantics → 35/35 GREEN. Instrumented: +8 ClipboardDatabaseTest cases (trailing-newline history / leading-ws pinned / internal-ws+newline todo persist verbatim; whitespace-removal round-trip; delete/unpin/todo-status after whitespace edit; whitespace-only content still `InvalidContent`), 2 stale trim-behavior tests updated to verbatim semantics (`ClipboardFeatureTest2.editEntry_whitespacePreservedVerbatim`, `ClipboardHistoryTest.testEditWhitespaceOnlyChangePersists`) — compile-gated (`compileDebugAndroidTestKotlin` green), ew run pending. Gates: compileDebugKotlin, runPureTests 1323, runMockTests 229. |
| **Final ew-cli gate #2** (post UT-1..UT-4 fixes) | — | 2026-07-15 | Full instrumented suite (Pixel7 API 34, orchestrator, 30m): **1343/1343 passed, 0 failures, 0 skips, 77 classes** (run 5ec54b9c-2673-4d36-9ede-28eedc79fc95). LauncherSelectKeyboardSafetyTest 2/2 (after compose-clock fix for the infinitely-animating FRE screen), AutocorrectTest 58/58, ClipboardDatabaseTest 100/100, tail classes (VocabularyRankingTest, WordPredictorTest) confirmed run. Prior run e8223f8a timed out at 25m/1286 tests with 1 test-harness failure — both root-caused (suite outgrew timeout; ComposeNotIdleException on perpetual animation) and fixed in 34a6b312a. Local: runPureTests 1323, runMockTests 229. |
