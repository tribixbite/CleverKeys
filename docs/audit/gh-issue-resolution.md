...
| Custom and unusual words are harder to swipe than they should be — especially custom words (wave U2) | **FIXED** | Root cause: the en lexicon's byte scale is 134..255 (floor measured over 98,140 words) but `CtcLexiconMerge` clamped stored custom frequencies into 1..255 RAW, and the Add-Word dialog defaulted to 100 with a fictional "1-10000" hint — so a default-added custom word ranked BELOW the entire English dictionary in the λ=4.0·ln(f) prior (tap side worse: raw 100 vs a 5.5K..1M binary scale). Fix: stored 1..255 user frequency is now CALIBRATED per consumer (`UserWordFrequency.scaleOnto`) — CTC merge maps [1..255]→[base_floor..255] (CKDT bases = identity), tap `WordPredictor` maps onto the loaded dictionary's span, the secondary index maps 255−stored→rank; dialogs are honest (1-255, default 255); `DictionaryManager.saveUserWords` no longer rewrites every stored frequency to 100 on add/remove; legacy stored values NOT rewritten (lifted at read time: 100→181 on en). Measured (`CtcCustomWordCalibrationReplayTest`, shipped encoder+beam, deterministic traces): name-like custom "bowien" @stored 100 went rank 3 (−0.14 vs "bowen") → rank 1 (+2.23); nonsense "flurble" margin 1.39 → 3.77 @100, 5.13 @255. Honest residual: BASE words at the 134 floor (e.g. "toml") keep the designed prior — straight-trace rank 2 to "time" by 0.018, forced-decode still +0.43 over "toll" — that gap is λ-by-design, not retuned (decoder params untouched). Fail-first: `CtcLexiconMergeTest` red 3/15 → OK 15; replay pin red → OK 4; suites green | — (no issue to comment on); maintainer soak: add a word via Dictionary Manager (default now 255) and swipe it |
...
|---|---|---|---|---|---|
| 148 | Clipboard opens without keyboard body / stuck behind nav bar (predictions off) | NOT_PLANNED (stale) | **FIXED** (wave U3; container half predated the audit) | Maintainer acknowledged the bug on-thread 2026-06-10, then stale bot closed it. Two halves: (1) the container gate — `PredictionViewSetup.kt:75` only built the top-pane containers when `word_prediction_enabled \|\| swipe_typing_enabled` — was already fixed by `cb7cebd4` (ARC-002, 2026-08-27; the audit's "root cause live at HEAD" claim was stale on this half); (2) the three `keyboard2.setInputView(<bare pane>)` fallbacks in KeyboardReceiver (+ the SWITCH_BACK restore) remained at HEAD — the exact replace-the-keyboard / behind-nav-bar symptom on any propagation regression. `56597c1b` deletes them: openers refuse without a container, no `setInputView` remains in the receiver. Pinned: `KeyboardReceiverPaneHostTest` (mock, hosted + refusal + toggle-reset), `CoreImeHygieneDriftTest` (no-setInputView + container-shape pins) | maintainer: reopen + close as completed citing `cb7cebd4`+`56597c1b` (bot recorded "not planned" on an acknowledged bug); ask reporter to retest with predictions+swipe both off |
| 149 | GIF zip pack inserts broken giphy link | NOT_PLANNED (stale) | **FIXED** (wave U3) | Reporter's dead URL has an all-lowercase ID — actually a compound keyword ("cute"+ID): the pipeline lowercased the whole slug AND keyword extraction appended compounds after the ID, so `getGiphyUrl()` rebuilt garbage. Fixed `ecd1abc8`+`56597c1b`: taps now commit the locally-shipped animated WebP via commitContent (works for EVERY pack incl. already-imported; app stays INTERNET-free), falling back to a CASE-PRESERVED URL carried as an explicit `gid:<Id>` search_text token (pipeline + `Gif.getGiphyId()`; no schema bump). Legacy imported packs: original-case IDs unrecoverable → URL intentionally null (media path serves them); repack to regain the URL fallback | maintainer: reopen + close as completed citing `ecd1abc8`+`56597c1b`; note existing pack ZIPs need a rebuild for the URL fallback (media insertion works without it) |
...
### Reopen candidates (verified still-present at HEAD `40b26dca`; both FIXED by wave U3 2026-09-06 — see the table rows for commits and the corrected #148 mechanism)
