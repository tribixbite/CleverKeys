...
| G (geo) | ARC-027 OQ-9 overshoot clamp; ARC-028 OQ-10 ordering slack; ARC-029 OQ-11 reversal signal — each gated on local-corpus replay evidence (no blind accuracy changes); ARC-030 floors ride | **ARC-027/028/029 CLOSED 2026-09-01** (`3237d23b`/`16d3ea8d`/`80238617`): three mechanisms built default-off + measured via `GeoOqSweepTest` (250×3 synthetic grid on qwerty/dvorak/weird × 3 tiers + 8.5k-trace local real-corpus replay per knob) — **three measured DECLINES, zero default changes** (OQ-9 neutral ±0.1 pt; OQ-10 lifts SLOPPY but regresses CLEAN/TYPICAL every layout, though it is +3.0 top-1 on the REAL corpus → adaptive-gate follow-up recorded; OQ-11 premise refuted — reversals don't predict error and the posterior is already under-confident, ECE worsens). Evidence: ledger Wave-G section + spec As-Built notes. ARC-030-floors-context + verb inversions still pending |
...
| J | FULL ew-cli run: all owed instrumented items | **COMPLETE 2026-09-02**: 1,466 tests, 2 permanent bench reds only; 3 first-run reds were all TEST bugs (fixed `86d77a16`, rerun green) — zero production bugs; also exposed+fixed the guard's R8 metaspace gap (`f3eb3ad8`) |
| K (device) | adb verification on both phones | **COMPLETE**: Saga pass 2026-09-02 (`e87c5b97`); Pixel full pass + K2 2026-09-02 (ledger Waves J+K / K2 sections) incl. #148 confirmation and the ARC-112 on-device swipe proof. 2026-09-03: Saga upgraded to the Sep-2 ARC-112-inclusive release (md5 `b0850ae3…` matches the Pixel's) — both devices byte-identical, soak-ready; IME setting untouched (Gboard, user's own) |
| L | Final consolidation + maintainer report | **COMPLETE 2026-09-02** — ledger waves J+K + ARC-110/111; HANDOFF rewritten to post-campaign state; maintainer decision list delivered |
