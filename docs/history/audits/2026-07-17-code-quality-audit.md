...
| Engineering practices | **B−** | Top-decile test/drift discipline & reproducible builds; the 1,371-test ew-cli suite runs manually (emulator.wtf), **not CI-gated**; **~19.7 GB git history**, R8 disabled, 973-issue lint baseline never gates. | B+ |
...
- **The instrumented tests are not wired into CI** (corrected 2026-07-17, post owner review). The 1,371-test `src/androidTest` suite is **real and actively run** — manually via `ew-cli` on emulator.wtf (Pixel7/API34, `--use-orchestrator`, documented in `.claude/skills/ew-cli-testing.md`, full-suite runs logged in `memory/todo.md`). What was non-functional is the *GitHub Actions* story: `ui-testing.yml` never ran `connectedAndroidTest`, gated every `adb` command with `|| true`, and targeted the wrong package id — so nothing gated in CI, and `release.yml` had no test job. The accurate defect is **"a working instrumented suite that isn't CI-gated," not "no instrumented tests / releases ship untested."** (Both workflows are fixed in this remediation round; wiring `ew-cli` into CI needs an `EW_API_TOKEN` secret.)
...
| 4 | **P2** *(was P1; corrected)* | **A working 1,371-test instrumented suite is not CI-gated; `ui-testing.yml` asserts were cosmetic.** The suite runs manually via `ew-cli`/emulator.wtf before releases (logged in `memory/todo.md`), so "releases ship untested" was wrong — but no GitHub Actions workflow invoked it: `ui-testing.yml` never ran `connectedAndroidTest`, `|| true`'d every `adb` call, and used the wrong package id; `release.yml` had no test job. Remaining work = wire `ew-cli` into CI behind an `EW_API_TOKEN` secret. | `.github/workflows/ui-testing.yml`, `release.yml`, `.claude/skills/ew-cli-testing.md` | 0.5 day |
...
8. Wire the existing `ew-cli` instrumented suite into CI behind an `EW_API_TOKEN` secret (curated subset on PRs, full suite nightly); the `ui-testing.yml` smoke asserts and the `release.yml` pure+MockK gate are already fixed in this round (#4).
